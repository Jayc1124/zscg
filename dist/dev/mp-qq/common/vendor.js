...
let id = 0;
function initEventChannel(events, cache2 = true) {
  id++;
...
function populateParameters(fromRes, toRes) {
  ...
  const parameters = {
    ...
    uniPlatform: {}.UNI_SUB_PLATFORM || "mp-qq",
    deviceBrand,
    deviceModel: model,
    deviceType,
...
function initCreateSubpackageApp(parseAppOptions) {
  return function createApp2(vm) {
    ...
    if ({}.UNI_SUBPACKAGE) {
      (qq.$subpackages || (qq.$subpackages = {}))[{}.UNI_SUBPACKAGE] = {
        $vm: vm
      };
    }
  };
...
function initAppLifecycle(appOptions, vm) {
  ...
  if (isFunction$1(appOptions.onShow) && qq.onAppShow) {
    qq.onAppShow((args) => {
      vm.$callHook("onShow", args);
    });
  }
  if (isFunction$1(appOptions.onHide) && qq.onAppHide) {
    qq.onAppHide((args) => {
...
function getAppVm() {
  ...
  if ({}.UNI_SUBPACKAGE) {
    return qq.$subpackages[{}.UNI_SUBPACKAGE].$vm;
  }
  return getApp().$vm;
}
