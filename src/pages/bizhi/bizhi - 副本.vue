...
export default{
		...
		methods:{
			detail(id,lx){
				...
				if(lx==1){
					uni.navigateTo({
						url:'/pages/bizhi/detail?id='+id
					})
				}else if(lx==2){
					uni.navigateTo({
						url:'/pages/bizhi/info?id='+id
					})
				}else if(lx==3){
					uni.navigateTo({
						url:'/pages/bizhi/info?id='+id
					...
					uni.navigateTo({
						url:'/pages/bizhi/infotp?id='+id
					})
				}else{
					uni.navigateTo({
						url:'/pages/bizhi/list?id='+id
					})
				}
			},
			onPageChange(e) {
			...
			getList() {
				...
				uni.request({
					url: 'https://lhsk.demo.hongcd.com/api/video/lists',
					data: data,
					success: data => {
