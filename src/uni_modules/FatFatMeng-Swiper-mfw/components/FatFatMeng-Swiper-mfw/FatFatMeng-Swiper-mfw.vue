	...
	<view class="Swiper-mfw-index-box">
		<view class="Swiper-mfw-index Swiper-box">
			<swiper class="Swiper-mfw" :circular="true" :indicator-dots="false" :autoplay="autoplay" :interval="3000"
				...
				<!-- 只需要前5条数据 -->
				<swiper-item class="swiper-mfw-item" v-for="(item,index) in list" :key="index">
					<!-- /*
					 1. 这里不需要用api控制暂停视频
				...
				<view class="Top-date-hot">
					...
					<view class="conter-date-nianyue">
						...
						<view class="right-date-nianyue">
							...
							<text class="Bottom-nian text">2022</text>
						</view>
					</view>
					<!-- 右边盒子 - 今日首推 -->
				...
				<view class="Bottom-datall">
					...
					<view class="Swiper-indicator-Top">
						<view class="Top-item" 
							...
							<view v-if="current==index" :class="current==index ? 'current':'no' ">
							<!-- 如果存在视频，则显示“视频预览”提示 -->
					...
					<view class="Swiper-indicator-Bottom">
						...
						<view class="Bottom-left-Imagelist">
							...
							<view class="Bottom-item" 
								...
								<image v-if="Number(index) <= 4" :class="current==index ? 'current':'no'" :src="item.Image" class="Image" mode="aspectFill"></image>
							</view>
						</view>
						<!-- 右边 -->
